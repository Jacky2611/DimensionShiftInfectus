...
import net.dimensionshiftinfectus.mod.world.generation.GenLayerDimensionShiftInfectus;
...
public class WorldChunkManagerInfectus extends WorldChunkManager {
	private GenLayer myGenBiomes;
	private GenLayer myBiomeIndexLayer;
	private BiomeCache myBiomeCache;
	private List<BiomeGenBase> myBiomesToSpawnIn;

	protected WorldChunkManagerInfectus() {
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList<BiomeGenBase>();
		this.myBiomesToSpawnIn.add(DimensionShiftInfectus.biomeWasteland);
		this.myBiomesToSpawnIn.add(DimensionShiftInfectus.biomeWastelandHilly);
		// this.myBiomesToSpawnIn.add(MainBiomes.TutorialBiomeEmerald);
		// this.myBiomesToSpawnIn.add(MainBiomes.TutorialBiomeSteel);
	}

	public WorldChunkManagerInfectus(long seed, WorldType worldtype) {
		this();
		// i changed this to my GenLayerTutorial
		GenLayer[] agenlayer = GenLayerDimensionShiftInfectus.makeTheWorld(seed);
		this.myGenBiomes = agenlayer[0];
		this.myBiomeIndexLayer = agenlayer[1];
	}

	public WorldChunkManagerInfectus(World world) {
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		if (biome == null) {
			return DimensionShiftInfectus.biomeWasteland;// default biome
		}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int par2, int par3, int par4, int par5) {
		...
		for (int i = 0; i < par4 * par5; ++i) {
			if (aint[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = DimensionShiftInfectus.biomeWasteland;
			}
		}
